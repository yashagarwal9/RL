...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(shape = [None, self.observation_dim], dtype = tf.float32)# TODO
    if self.discrete:
      self.action_placeholder = tf.placeholder(shape = [None,], dtype = tf.int32)# TODO
    else:
      self.action_placeholder = tf.placeholder(shape = [None, self.action_dim], dtype = tf.float32)# TODO

    # Define a placeholder for advantages
    self.advantage_placeholder = tf.placeholder(shape = [None,], dtype = tf.float32)# TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      print(tf.shape(action_logits))
      self.sampled_action =   tf.squeeze(tf.multinomial(action_logits, 1))# TODO
      print(tf.shape(self.action_placeholder))
      ...
      self.sampled_action = tf.random_normal([1,], action_means, tf.exp(log_std))  # TODO
      self.logprob = tf.contrib.distributions.MultivariateNormalDiag(action_means, log_std)         # TODO
      self.logprob = self.logprob.log_prob(self.action_placeholder)
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 
      						  ...
      						  self.config.layer_size,
      						  output_activation = None))# TODO
    self.baseline_target_placeholder = tf.placeholder(shape = [None,], dtype = tf.float32)# TODO
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for timestep in range(len(rewards)):
      	...
      	g_i = 0
      	for i in range(len(rewards) - timestep):
      		g_i += self.config.gamma**i * reward[i]
      	p_returns.append(g_i)# TODO
      #######################################################
      #########          END YOUR CODE.          ############
      all_returns.append(p_returns)
  ...
  def calculate_advantage(self, returns, observations):
    ...
    #########   YOUR CODE HERE - 5-10 lines.   ############
    if self.config.use_baseline:
      baseline = self.sess.run(self.baseline, feed_dict = {self.observation_placeholder: observations})# TODO
      adv -= baseline.squeeze() 
    if self.config.normalize_advantage:
      adv = (adv - adv.mean() / adv.std())# TODO
    #######################################################
    #########          END YOUR CODE.          ############
    return adv
